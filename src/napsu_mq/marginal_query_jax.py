...
class MarginalQuery:
    def __init__(self, inds: Iterable[int], value: Iterable[int], features: Optional[Iterable] = None):
        ...
        self.value = value if type(value) is jnp.ndarray else jnp.array(value)
...
class FullMarginalQuerySet:
    ...
    def query(self, x: jnp.ndarray) -> Dict:
    ...
    def query_sum(self, x: jnp.ndarray) -> Dict:
    ...
    def query_feature_set(self, feature_set: Tuple, x: jnp.ndarray) -> jnp.ndarray:
    ...
    def query_feature_set_sum(self, feature_set: Tuple, x: jnp.ndarray) -> jnp.ndarray:
    ...
    def get_canonical_queries(self) -> 'FullMarginalQuerySet':
        ...
        base_value = jnp.zeros(d, dtype=int)
        ...
        print(f"Calculating canonical queries, clique_set length: {len(clique_set)}")
        for clique in tqdm(clique_set):
            clique = set(clique)
            clique_ordered = tuple(clique)
            if clique == set():
                continue
            index_conversion = jnp.full((d,), -1)
            for i, variable in enumerate(clique_ordered):
                index_conversion.at[variable].set(i)
            conv_clique_indices = index_conversion[jnp.array(list(clique))]
            ...
            for val in tqdm(clique_product):
                value = jnp.zeros(len(clique), dtype=int)
                value.at[conv_clique_indices].set(jnp.asarray(val, dtype=int))
                counter = jnp.zeros(tuple([len(self.values_by_int_feature[variable]) for variable in clique_ordered]),
                                   dtype=int)
                subsets = powerset(clique)
                for subset in subsets:
                    multiplier = (-1) ** (len(clique) - len(subset))
                    base_indices = list(clique.difference(subset))
                    conv_base_indices = index_conversion[jnp.array(base_indices)]
                    completed_value = value.copy()
                    completed_value.at[conv_base_indices].set(base_value[conv_base_indices])
                    counter.at[tuple(completed_value)].set(counter[tuple(completed_value)] + multiplier)
        ...
        print(f"Calculating new queries, not_original_clique_queries length: {len(not_original_clique_queries)}")
...
class QueryList:
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        return jnp.concatenate([query(x) for query in self.queries], axis=1)

    def __str__(self):
        return "\n".join(query.__str__() for query in self.queries)

    def evaluate_subset(self, x: jnp.ndarray, subset: Iterable[int]) -> jnp.ndarray:
        ...
        Returns:
            np.ndarray: The query results.
        """
        return jnp.concatenate([self.queries[i](x) for i in subset], axis=1)
