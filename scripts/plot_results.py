...
if "logistic" in original_results_csv and "logistic" in synthetic_results_csv:
    ...
    for dataset_name in dataset_names:
        ...
        for index in range(n_index):
            ...
            g = sns.FacetGrid(combined_data, col="MCMC_algorithm")
            g.map(sns.barplot, "epsilon", "accuracy")

            g.savefig(f"plots/lr_comparison_{index}.svg")


elif "classification" in original_results_csv and "classification" in synthetic_results_csv:
    n_original_datasets = original_data['dataset_name'].nunique()
    dataset_names = original_data['dataset_name'].unique()
    ...
    raise ValueError(f"CSV files not recognized: {original_results_csv}, {synthetic_results_csv}")
