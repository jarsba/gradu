...
from src.utils.preprocess_dataset import clean_synthetic_adult
...
if synthetic_task:
    ...
    results = pd.DataFrame(columns=["dataset_name", "accuracy", "balanced_accuracy", "F1", "coefficients", "point_estimates", "variance_estimates", "confidence_intervals"])
...
for path in dataset_paths:
    ...
    point_estimates, variance_estimates, confidence_intervals = logistic_regression(
        df_np, col_to_predict=10, add_constant=False, return_intervals=True, conf_levels=[0.95])

    if synthetic_task:
        experiment_id, dataset_index, _, query, epsilon, MCMC_algorithm = get_metadata_from_synthetic_path(path)
        results.append([experiment_id, dataset_name, dataset_index, query, epsilon, MCMC_algorithm, accuracy_score, balanced_accuracy_score, f1_score, coeficcients, point_estimates, variance_estimates, confidence_intervals])
    else:
        results.append([dataset_name, accuracy_score, balanced_accuracy_score, f1_score, coeficcients, point_estimates, variance_estimates, confidence_intervals])

if synthetic_task:
    synthetic_array = np.dstack(df_np_arrays)
    logistic_regression(synthetic_array, col_to_predict=10, add_constant=False, return_intervals=True, conf_levels=[0.95])
