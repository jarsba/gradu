# Originally from https://github.com/PROBIC/dp-bayes-theory/blob/master/maximum-entropy-mechanism/lib/plot_dataframe_marginal_comparison.py by Ossi Räisä
...
def plot_dataframe_marginal_comparison(orig_df, syn_dfs, filename=None, estimator=np.mean):
    ...
    for i, (key, value) in enumerate(value_count_dfs.items()):
        ...
        # sns.stripplot(x="Value", y="Count", data=value_count_dfs[key], hue="Type", dodge=True, ax=ax)
